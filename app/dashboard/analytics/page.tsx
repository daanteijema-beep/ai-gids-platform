...
async function getAnalytics() {
  const [
    ...
    supabaseAdmin.from('pdf_analytics').select('*').order('created_at', { ascending: false }).limit(200),
  ])
  ...
  const totalRevenue = pdfStats.reduce((s, p) => s + p.revenue, 0)
  const totalSold = pdfStats.reduce((s, p) => s + p.sold, 0)
  ...
  for (const l of (learnings || [])) {
    if (!learningsByType[l.learning_type]) learningsByType[l.learning_type] = []
    learningsByType[l.learning_type]!.push(l)
  }
...
export default async function AnalyticsPage() {
  const { pdfStats, totalRevenue, totalSold, totalLeads, weeklyRevenue, learningsByType } =
    await getAnalytics()

  const weeks = Object.entries(weeklyRevenue).slice(-8)
  const maxWeekRevenue = Math.max(...weeks.map(([, v]) => v), 1)
    ...
    <div>
      ...
      {weeks.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-8">
          ...
          <div className="flex items-end gap-2 h-32">
            ...
            {weeks.length === 0 && (
              <p className="text-gray-400 text-sm">Nog geen sales</p>
            )}
          </div>
        </div>
      )}

      {/* PDF performance table */}
            ...
            <tbody className="divide-y divide-gray-100">
              {pdfStats.map((pdf) => (
                <tr key={pdf.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center text-gray-700">€{pdf.price}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      Number(pdf.conversion) >= 10 ? 'bg-green-100 text-green-700' :
                      Number(pdf.conversion) >= 3 ? 'bg-yellow-100 text-yellow-700' :
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a
            href={`/api/agents/orchestrate?secret=${process.env.CRON_SECRET}`}
            target="_blank"
              ...
              <div key={type}>
                ...
                <div className="space-y-2">
                  {items!.slice(0, 3).map((l, i) => (
                    <div key={i} className="text-sm bg-gray-50 rounded-lg px-4 py-2.5 text-gray-700">
                      {l.insight}
                    </div>
                  ))}
                </div>
